<?php
    ...
    if(isset($_GET['id'])){
        ...
        $query_get = "select * from manual_allowances where id = '$id'";
        $run_query_get = mysqli_query($conn, $query_get);
        $result = mysqli_fetch_object($run_query_get);

        if(!$result){
            header("location: manual-tunjangan.php");
        }
    }
    ...
    if(isset($_POST['simpan'])){
        ...
        $query_update = "update manual_allowances set userid = '$userid', name = '$name', amount = '$amount', payroll_month = '$payroll_month', payroll_year = '$payroll_year' where id = '$id'";
